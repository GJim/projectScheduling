...
function getResult() {
  ...
  if(cs.length > projects.length) {
    let ct = 0;
    while (ct < (cs.length - projects.length)) {
      let rnd = Math.floor((Math.random() * classrooms.length));
      if (crm[rnd].available === true) {
        crm[rnd].available = false;
        ct++;
      }
    }
  }
    ...
    function(data, status){
      ...
      for(let i=0; i<crm.length; i++) {
        if(crm[i].available) {
          ...
          $( "#rStage" + parseInt(i/(crm.length/4)+1) ).append('<td>'+p+t+'</td>');
          c++;
        } else {
          $( "#rStage" + parseInt(i/(crm.length/4)+1) ).append('<td></td>');
        }
      }
      let professors = $("#tInput").val().split(',');
      let pNumber = [];
